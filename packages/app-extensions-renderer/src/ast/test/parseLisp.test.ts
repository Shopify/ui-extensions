...
describe('parseLisp', () => {
  ...
  it('supports lists with elements', () => {
    const lisp = '(1 2 3)';
    expect(parseLisp(lisp)).toEqual(new List([new Literal(1), new Literal(2), new Literal(3)]));
  });

  it('supports expressions without arguments', () => {
    const lisp = '(alert)';
    expect(parseLisp(lisp)).toEqual(new List([new Identifier('alert')]));
  });
  ...
  it('supports expressions with sub-expressions', () => {
    ...
    expect(parseLisp(lisp)).toEqual(
      new List([
        new Identifier('sum'),
        new List([new Identifier('mul'), new Literal(1), new Literal(2)]),
        new List([new Identifier('mul'), new Literal(3), new Literal(4)]),
      ]),
    );
  });
