...
Your extension can pass a [session token](/docs/api/customer-account-ui-extensions/apis/order-status-api/session-token) to your API server but this only guarantees the integrity of its claims. It does not guarantee the request itself originated from Shopify. For example, your API server could trust the session token's \`sub\` claim (the customer ID) but it could not trust a \`?customer_id=\` query parameter.

> Note:
> [Session token](/docs/api/customer-account-ui-extensions/apis/order-status-api/session-token) is only available to order status [targets](/api/customer-account-ui-extensions/targets).
`,
        },
      ],
    },
