...
interface ClickableChipProps$1 extends ChipProps$1, GlobalProps, InteractionProps {
	/**
...
export type ListSlotType = "primary" | "secondary" | "kicker" | "inline" | "labeled";
...
export declare abstract class PreactCustomElement extends BaseClass {	/** @private */
...
export type CallbackToggleEvent<TTagName extends keyof HTMLElementTagNameMap, TEvent extends ToggleEvent = ToggleEvent> = TEvent & {
    currentTarget: HTMLElementTagNameMap[TTagName];
};
export type CallbackEventListener<T extends keyof HTMLElementTagNameMap> = (EventListener & {
    (event: CallbackEvent<T>): void;
}) | null;
export type CallbackErrorEventListener<TTagName extends keyof HTMLElementTagNameMap, TError extends Error = Error> = (EventListener & {
    (event: CallbackEvent<TTagName> & {
        error: TError;
    }): void;
}) | null;
export interface CallbackExtendableEvent<TTagName extends keyof HTMLElementTagNameMap> extends CallbackEvent<TTagName>, Pick<ExtendableEvent, 'waitUntil'> {
}
export type CallbackExtendableEventListener<TTagName extends keyof HTMLElementTagNameMap> = (EventListener & {
...
export interface ReactProps$_ extends Partial<AvatarProps>, Pick<AvatarProps$1, 'id'> {
    onLoad?: () => void;
    onError?: () => void;
}

export interface IconProps extends Required<Pick<IconProps$1, 'type' | 'tone' | 'color' | 'size' | 'interestFor'>> {
    /**
...
export interface BadgeProps extends Pick<BadgeProps$1, 'color' | 'icon' | 'size' | 'tone'> {
...
export interface BadgeJSXProps extends Partial<BadgeProps>, Pick<BadgeProps$1, 'id' | 'children'> {
    /**
     * The content of the Badge.
     */
    children?: ComponentChildren;
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$Y]: Omit<BannerJSXProps, 'secondaryActions'> & PreactBaseElementPropsWithChildren<Banner>;
        }
    }
}

declare const tagName$Y = "s-banner";
export interface BannerJSXProps extends Partial<BannerProps>, Pick<BannerProps$1, 'id' | 'children'> {
    /**
     * The content of the Banner.
     */
    children?: ComponentChildren;
...
export type ResponsiveBoxProps = MakeResponsivePick<RequiredBoxProps, 'padding' | 'paddingBlock' | 'paddingBlockStart' | 'paddingBlockEnd' | 'paddingInline' | 'paddingInlineStart' | 'paddingInlineEnd' | 'display'>;
export interface BoxProps extends Pick<RequiredBoxProps, 'accessibilityLabel' | 'accessibilityRole' | 'accessibilityVisibility' | 'background' | 'blockSize' | 'border' | 'borderColor' | 'borderRadius' | 'borderStyle' | 'borderWidth' | 'inlineSize' | 'maxBlockSize' | 'maxInlineSize' | 'minBlockSize' | 'minInlineSize' | 'overflow'> {
    /**
...
export interface BoxJSXProps extends Partial<BoxProps>, Pick<BoxProps$1, 'id' | 'children'> {
    /**
     * The content of the Box.
     */
    children?: ComponentChildren;
...
export type ButtonBaseProps = Required<Pick<ButtonOnlyProps, 'accessibilityLabel' | 'disabled' | 'command' | 'commandFor' | 'icon' | 'interestFor' | 'lang' | 'loading' | 'type' | 'tone' | 'variant' | 'target' | 'href' | 'download' | 'inlineSize'>>;
export interface ButtonProps extends ButtonBaseProps {
    tone: Extract<ButtonProps$1['tone'], 'neutral' | 'critical' | 'auto'>;
    icon: IconProps['type'];
}

export interface PreactOverlayControlProps extends Pick<InteractionProps, 'commandFor' | 'interestFor'> {
    /**
...
declare class Button extends ButtonBase<typeof tagName$W> implements ButtonProps {
...
export interface ButtonGroupProps extends Required<Pick<ButtonGroupProps$1, 'gap' | 'accessibilityLabel'>> {
}
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$V]: Omit<ButtonGroupJSXProps, 'primaryAction' | 'secondaryActions'> & PreactBaseElementPropsWithChildren<ButtonGroup>;
        }
    }
}

declare const tagName$V = "s-button-group";
export interface ButtonGroupJSXProps extends Partial<ButtonGroupProps>, Pick<ButtonGroupProps$1, 'id' | 'children'> {
    /**
     * The content of the ButtonGroup.
     */
    children?: ComponentChildren;
...
export type PreactInputProps = Required<Pick<TextFieldProps$1, 'disabled' | 'id' | 'name' | 'value'>>;
...
export interface PreactCheckboxProps extends Required<Pick<CheckboxProps$1, 'accessibilityLabel' | 'checked' | 'defaultChecked' | 'details' | 'error' | 'label' | 'required' | 'name' | 'disabled'>> {
...
declare class PreactCheckboxElement extends PreactInputElement implements PreactCheckboxProps {
    /**
     * Whether the checkbox is currently selected. When `true`, the checkbox displays a checkmark
     * and its `value` is included in form submissions. Bind to this property to track or control
     * the selection state programmatically. Falls back to `defaultChecked` if not explicitly set.
     */
    get checked(): boolean;
...
export interface CheckboxProps extends PreactCheckboxProps {
    /**
     * Whether the checkbox is currently in an indeterminate (partially selected) state. The
     * indeterminate state is visual only—it does not affect the `checked` value. Commonly used
     * for parent checkboxes that represent a group where some but not all children are selected.
     */
    indeterminate: Required<CheckboxProps$1>['indeterminate'];
    /**
     * Whether the checkbox displays in an indeterminate (partially selected) state on initial
     * render. The indeterminate state is visual only and does not affect the `checked` value.
     * Useful for parent checkboxes that represent a group where some children are selected by
     * default.
     */
...
export interface ChipProps extends Required<Pick<ChipProps$2, 'color' | 'accessibilityLabel'>> {
}
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$T]: Omit<ChipJSXProps, 'graphic'> & PreactBaseElementPropsWithChildren<Chip>;
        }
    }
}

declare const tagName$T = "s-chip";
export interface ChipJSXProps extends Partial<ChipProps>, Pick<ChipProps$2, 'id' | 'children'> {
    /**
     * The content of the Chip.
     */
    children?: ComponentChildren;
...
export interface ChoiceProps extends Required<Pick<ChoiceProps$1, 'selected' | 'defaultSelected' | 'disabled' | 'accessibilityLabel' | 'value'>> {
}
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$S]: Omit<ChoiceJSXProps, 'details' | 'secondaryContent'> & PreactBaseElementPropsWithChildren<Choice>;
        }
    }
}

declare const tagName$S = "s-choice";
export interface ChoiceJSXProps extends Partial<ChoiceProps>, Pick<ChoiceProps$1, 'id' | 'children' | 'details'> {
    /**
     * Content to use as the choice label.
     *
...
export interface ChoiceListProps extends Required<Pick<ChoiceListProps$1, 'details' | 'disabled' | 'error' | 'label' | 'labelAccessibilityVisibility' | 'multiple' | 'name' | 'values'>> {
}
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$R]: ChoiceListJSXProps & PreactBaseElementPropsWithChildren<ChoiceList>;
        }
    }
}

declare const tagName$R = "s-choice-list";
export interface ChoiceListJSXProps extends Partial<ChoiceListProps>, Pick<ChoiceListProps$1, 'id' | 'children'> {
    /**
     * The choices a user can select from.
     *
     * Accepts `Choice` components.
...
export type ClickableBaseProps = Required<Pick<ClickableProps$1, 'command' | 'commandFor' | 'interestFor' | 'disabled' | 'download' | 'href' | 'lang' | 'loading' | 'overflow' | 'target' | 'type'>>;
export interface ClickableProps extends Required<BoxProps>, ClickableBaseProps {
}

declare const Clickable_base: (abstract new (renderImpl: _shopify_admin_web_component_foundations.RenderImpl) => BoxElement & PreactOverlayControlProps) & Pick<typeof BoxElement, "prototype" | "observedAttributes">;
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$Q]: ClickableJSXProps & PreactBaseElementPropsWithChildren<Clickable>;
        }
    }
}

declare const tagName$Q = "s-clickable";
export interface ClickableJSXProps extends Partial<ClickableProps>, Pick<ClickableProps$1, 'id' | 'children'> {
    /**
...
export interface ClickableChipProps extends Required<Pick<ClickableChipProps$1, 'color' | 'accessibilityLabel' | 'removable' | 'hidden' | 'href' | 'disabled' | 'command' | 'commandFor' | 'interestFor'>> {
}

declare const ClickableChip_base: (abstract new (renderImpl: Omit<_shopify_admin_web_component_foundations.RenderImpl, "globalShadowCSS">) => PolarisCustomElement & PreactOverlayControlProps) & Pick<typeof PolarisCustomElement, "prototype" | "observedAttributes">;
declare class ClickableChip extends ClickableChip_base implements ClickableChipProps {
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$P]: Omit<ClickableChipJSXProps, 'graphic'> & PreactBaseElementPropsWithChildren<ClickableChip>;
        }
    }
}

declare const tagName$P = "s-clickable-chip";
export interface ClickableChipJSXProps extends Partial<ClickableChipProps>, Pick<ClickableChipProps$1, 'id' | 'children'> {
    /**
...
export type PreactFieldProps<Autocomplete extends string = string> = PreactInputProps & Required<Pick<TextFieldProps$1, 'defaultValue' | 'details' | 'error' | 'label' | 'labelAccessibilityVisibility' | 'placeholder' | 'readOnly' | 'required'>> & {
    /**
     * A hint as to the intended content of the field.
     *
     * When set to `on` (the default), this property indicates that the field should support
...
declare class PreactFieldElement<Autocomplete extends string = string> extends PreactInputElement implements PreactFieldProps<Autocomplete> {
    ...
    /**
     * The initial value of the field when the component mounts. Unlike `value`, this only sets
     * the starting state and does not update on subsequent changes. When the form is reset, the
     * field returns to this value. Useful for pre-populating fields with existing data.
     */
    defaultValue: PreactFieldProps['defaultValue'];
    /**
     * Additional help text displayed below the field, providing context or guidance about the
     * expected input. Unlike `error`, this text is always visible regardless of validation state.
     */
    details: PreactFieldProps['details'];
    /**
     * Error message displayed below the field when validation fails. When set to a non-empty
     * string, the field displays in an error state with the message visible to users.
     */
    error: PreactFieldProps['error'];
    /**
     * The visible text label displayed above the field, describing what information should be
     * entered. The label is associated with the input for accessibility. Required for accessibility
     * unless `accessibilityLabel` is provided.
     */
...
export type ColorFieldProps = PreactFieldProps<Required<ColorFieldProps$1>['autocomplete']> & Required<Pick<ColorFieldProps$1, 'alpha' | 'value' | 'defaultValue'>>;
...
export interface ColorFieldJSXProps extends Partial<Omit<ColorFieldProps, 'accessory'>>, Pick<ColorFieldProps$1, 'id' | 'alpha' | 'value' | 'defaultValue'>, FieldReactProps<typeof tagName$O> {
    onInput?: (event: CallbackEvent<typeof tagName$O>) => void;
    onChange?: (event: CallbackEvent<typeof tagName$O>) => void;
}

export interface ColorPickerProps extends Required<Pick<ColorPickerProps$1, 'id' | 'alpha' | 'value' | 'defaultValue' | 'name'>> {
}
...
export interface ColorPickerJSXProps extends Partial<ColorPickerProps>, Pick<ColorPickerProps$1, 'id' | 'alpha' | 'value' | 'defaultValue' | 'name'> {
    onInput?: (event: CallbackEvent<typeof tagName$N>) => void | null;
    onChange?: (event: CallbackEvent<typeof tagName$N>) => void | null;
}

export interface DateFieldProps extends PreactFieldProps<DateAutocompleteField>, Required<Pick<DateFieldProps$1, 'allow' | 'allowDays' | 'disallow' | 'disallowDays' | 'value' | 'defaultValue' | 'view' | 'defaultView'>> {
}
...
export interface DateFieldJSXProps extends Partial<DateFieldProps>, Pick<DateFieldProps$1, 'id'> {
...
export interface DatePickerProps extends Required<Pick<DatePickerProps$1, 'defaultView' | 'view' | 'allow' | 'disallow' | 'allowDays' | 'disallowDays' | 'value' | 'defaultValue' | 'name'>> {
...
export type RequiredAlignedDropZoneProps = Required<DropZoneProps$1>;
...
declare abstract class DropZoneBase extends PolarisCustomElement {
    static formAssociated: boolean;
    /**
     * Specifies which file types the drop zone accepts. Uses standard MIME types (such as
     * `'image/*'`, `'application/pdf'`) or file extensions (such as `'.csv'`). Supports
     * comma-separated values for multiple types. When set, non-matching files are rejected
     * and trigger `ondroprejected`. When empty, all file types are accepted.
     */
    ...
    /**
     * Callback fired when the selected files change, either through the file picker dialog or
     * drag-and-drop. Fires after files are validated against the `accept` criteria. Access the
     * selected files via `event.currentTarget.files`. This is the primary event for handling
     * file selection—useful for processing uploads, updating UI state, or triggering validation.
     */
    onchange: CallbackEventListener<typeof tagName$J>;
    /**
     * Callback fired during file input. Similar to `onchange` but might fire more frequently during
     * the selection process. Provided for compatibility with form libraries that expect standard
     * input events.
     */
    oninput: CallbackEventListener<typeof tagName$J>;
    /**
     * Callback fired when a user attempts to drop or select files that don't match the `accept`
     * criteria. The event provides information about which files were rejected and why. Useful
     * for displaying specific error messages like "Only PNG and JPEG images are accepted".
     */
    ondroprejected: CallbackEventListener<typeof tagName$J>;
    get value(): string;
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$J]: DropZoneJSXProps & PreactBaseElementPropsWithChildren<DropZone>;
        }
    }
}

declare const tagName$J = "s-drop-zone";
export interface DropZoneJSXProps extends Partial<DropZoneProps>, Pick<DropZoneProps$1, 'id'> {
    /**
     * Custom content displayed inside the drop zone area, replacing the default upload prompt.
     * Can include custom instructions, file previews, upload progress indicators, or any other
     * elements. When files are selected, this slot can display thumbnails, file names, or other
...
export type EmailFieldProps = PreactFieldProps<Required<EmailFieldProps$1>['autocomplete']> & Required<Pick<EmailFieldProps$1, 'maxLength' | 'minLength'>>;
...
export interface EmailFieldJSXProps extends Partial<Omit<EmailFieldProps, 'accessory'>>, Pick<EmailFieldProps$1, 'id'>, FieldReactProps<typeof tagName$I> {
}

export type RequiredAlignedProps = Required<GridProps$1>;
export type ResponsiveGridProps = MakeResponsivePick<RequiredAlignedProps, 'rowGap' | 'columnGap' | 'gap' | 'gridTemplateColumns' | 'gridTemplateRows'>;
export interface GridProps extends BoxProps, Required<Pick<GridProps$1, 'alignItems' | 'justifyItems' | 'placeItems' | 'alignContent' | 'justifyContent' | 'placeContent'>> {
    /**
...
export interface GridJSXProps extends Partial<GridProps>, Pick<GridProps$1, 'id' | 'children'> {
    /**
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$G]: GridItemJSXProps & PreactBaseElementPropsWithChildren<GridItem>;
        }
    }
}

declare const tagName$G = "s-grid-item";
export interface GridItemJSXProps extends Partial<GridItemProps>, Pick<GridItemProps$1, 'id' | 'children'> {
    /**
...
export interface HeadingProps extends Required<Pick<HeadingProps$1, 'accessibilityRole' | 'accessibilityVisibility' | 'lineClamp'>> {
}
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$F]: HeadingJSXProps & PreactBaseElementPropsWithChildren<Heading>;
        }
    }
}

declare const tagName$F = "s-heading";
export interface HeadingJSXProps extends Partial<HeadingProps>, Pick<HeadingProps$1, 'id' | 'children'> {
    /**
     * The content of the Heading.
     */
    children?: ComponentChildren;
...
export interface IconJSXProps extends Partial<IconProps>, Pick<IconProps$1, 'id'> {
}

export interface ImageProps extends Required<Pick<ImageProps$1, 'alt' | 'loading' | 'src' | 'accessibilityRole' | 'inlineSize' | 'srcSet' | 'sizes' | 'aspectRatio' | 'objectFit'>>, Required<Pick<BoxProps, 'border' | 'borderColor' | 'borderRadius' | 'borderStyle' | 'borderWidth'>> {
}
...
export type LinkBaseProps = Required<Pick<LinkProps$1, 'accessibilityLabel' | 'command' | 'commandFor' | 'interestFor' | 'download' | 'href' | 'lang' | 'target' | 'tone'>>;
...
declare abstract class LinkBase<TTagName extends keyof HTMLElementTagNameMap> extends LinkBase_base implements Pick<LinkProps, 'accessibilityLabel' | 'href' | 'target' | 'download' | 'lang'> {
...
declare class Link extends LinkBase<typeof tagName$C> implements LinkProps {
    tone: LinkProps['tone'];
    constructor();
}
declare global {
...
export interface LinkJSXProps extends Partial<LinkProps>, Pick<LinkProps$1, 'id' | 'lang' | 'children'> {
    /**
     * The content of the Link.
     */
    children?: ComponentChildren;
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$B]: ListItemJSXProps & PreactBaseElementPropsWithChildren<ListItem>;
        }
    }
}

declare const tagName$B = "s-list-item";
export interface ListItemJSXProps extends Partial<ListItemProps>, Pick<ListItemProps$1, 'id' | 'children'> {
    /**
     * The content of the ListItem.
     */
    children?: ComponentChildren;
...
export interface MenuProps extends Required<Pick<MenuProps$1, 'id' | 'accessibilityLabel'>> {
}
...
export interface MenuJSXProps extends Partial<MenuProps>, Pick<MenuProps$1, 'id' | 'children'> {
    /**
     * The Menu items.
     *
...
export interface ModalProps extends Pick<RequiredAlignedModalProps, 'hideOverlay' | 'showOverlay' | 'toggleOverlay'> {
    /**
...
declare abstract class ModalBase<TTagName extends keyof HTMLElementTagNameMap> extends PreactOverlayElement implements Pick<ModalProps, 'accessibilityLabel' | 'heading' | 'padding' | 'size'> {
    ...
    /**
     * Callback fired immediately when the modal begins to close, before any exit animations start.
     * The modal is still visible at this point. Useful for canceling pending requests or beginning
     * cleanup operations immediately. For callbacks that should run after the modal is fully hidden,
     * use `onafterhide` instead.
     */
    onhide: CallbackEventListener<TTagName> | null;
    /**
     * Callback fired immediately when the modal begins to open, before any entrance animations start.
     * The modal may not be fully visible or interactive at this point. Useful for initializing state
     * or fetching data that should be ready when the animation completes. For callbacks that should
     * run after the modal is fully visible, use `onaftershow` instead.
     */
    onshow: CallbackEventListener<TTagName> | null;
    /**
     * Callback fired after the modal is fully hidden and all exit animations have completed. Focus
     * has been restored to the element that triggered the modal. Useful for navigation, showing
     * success notifications, or final cleanup.
     */
    onafterhide: CallbackEventListener<TTagName> | null;
    /**
     * Callback fired after the modal is fully visible and all entrance animations have completed.
     * The modal is interactive and focus has been moved inside. Useful for focusing a specific
     * input field or triggering analytics events.
     */
    ...
    [hide](): Promise<void>;
    /**
     * Programmatically opens the modal. Use when opening the modal based on application logic
     * rather than a direct user click, such as after an async operation completes or when
     * responding to a keyboard shortcut. For simple button triggers, prefer using `commandFor`
     * attribute declaratively. The method is idempotent—calling it when the modal is already
     * open has no effect.
     */
    showOverlay(): void;
    /**
     * Programmatically closes the modal. Use when closing the modal based on application logic,
     * such as after a form submission succeeds or when a timeout expires. Focus automatically
     * returns to the element that opened the modal. The method is idempotent—calling it when
     * the modal is already closed has no effect. For close buttons inside the modal, prefer
     * using `commandFor` with `command="--hide"` declaratively.
     */
    hideOverlay(): void;
    /**
     * Programmatically toggles the modal's visibility—opens it if closed, closes it if open.
     * Use when a single control should alternate between showing and hiding the modal.
     * Equivalent to calling `showOverlay()` when closed or `hideOverlay()` when open.
     * For most use cases, explicit `showOverlay()` or `hideOverlay()` calls provide clearer intent.
     */
    toggleOverlay(): void;
    /** @private */
    connectedCallback(): void;
...
declare class Modal extends ModalBase<typeof tagName$z> implements ModalProps {
    constructor();
}
declare global {
    interface HTMLElementTagNameMap {
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$z]: Omit<ModalJSXProps, 'primaryAction' | 'secondaryActions'> & PreactBaseElementPropsWithChildren<Modal>;
        }
    }
}

declare const tagName$z = "s-modal";
export interface ModalJSXProps extends Partial<ModalProps>, Pick<ModalProps$1, 'id' | 'children'> {
    /**
     * The main content area of the modal, rendered between the header and footer. Accepts any
     * valid elements including text, forms, lists, or other components. Content scrolls
     * automatically when it exceeds the available height within the modal. The modal is opened
     * by a trigger element (typically a button) that references this modal's `id` via the
     * `commandFor` attribute.
     */
    children?: ComponentChildren;
    /**
     * Slot for the modal's main call-to-action button, rendered in the footer on the right side.
     * Accepts an `s-button` element. To close the modal when clicked, include `commandFor` pointing
     * to the modal's `id` and `command="--hide"`. For async operations like saving data, handle
     * closing programmatically after the operation completes. Common labels include "Save", "Delete",
     * or "Confirm".
     */
    primaryAction?: ComponentChildren;
    /**
     * Slot for additional action buttons, rendered in the footer to the left of the primary action.
     * Accepts `s-button` elements. Multiple buttons are supported and arranged horizontally.
     * To close the modal from a button, include `commandFor` pointing to the modal's `id` and
     * `command="--hide"`. Common labels include "Cancel" or "Back".
     */
    secondaryActions?: ComponentChildren;
    /**
     * Callback fired immediately when the modal begins to close, before any exit animations start.
...
export interface MoneyFieldProps extends Omit<PreactFieldProps, 'value'>, Pick<RequiredMoneyFieldProps, 'max' | 'min'> {
...
export interface MoneyFieldJSXProps extends Partial<MoneyFieldProps>, FieldReactProps<typeof tagName$y>, Pick<MoneyFieldProps$1, 'id'> {
}

export interface NumberFieldProps extends Omit<PreactFieldProps<Required<NumberFieldProps$1>['autocomplete']>, 'value'>, Required<Pick<NumberFieldProps$1, 'inputMode' | 'max' | 'min' | 'prefix' | 'step' | 'suffix'>> {
...
export interface NumberFieldJSXProps extends Partial<NumberFieldProps>, Pick<NumberFieldProps$1, 'id'>, FieldReactProps<typeof tagName$x> {
}

export interface OptionProps extends Required<Pick<OptionProps$1, 'disabled' | 'value' | 'selected' | 'defaultSelected'>> {
}
...
export interface OptionJSXProps extends Partial<OptionProps>, Pick<OptionProps$1, 'id' | 'children'> {
    /**
     * The content to use as the label.
     */
    children?: ComponentChildren;
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$v]: OptionGroupJSXProps & PreactBaseElementPropsWithChildren<OptionGroup>;
        }
    }
}

declare const tagName$v = "s-option-group";
export interface OptionGroupJSXProps extends Partial<OptionGroupProps>, Pick<OptionGroupProps$1, 'id' | 'children'> {
    /**
     * The options a user can select from.
     *
     * Accepts `Option` components.
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$u]: OrderedListJSXProps & PreactBaseElementPropsWithChildren<OrderedList>;
        }
    }
}
...
export type RequiredAlignedPageProps = Required<PageProps$1>;
export interface PageProps {
     ...
     * resource name (e.g., "Order #1001") helps provide context. The heading is used for
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$t]: Omit<PageJSXProps, 'aside' | 'primaryAction' | 'secondaryActions' | 'breadcrumbActions'> & PreactBaseElementPropsWithChildren<Page>;
        }
    }
}

declare const tagName$t = "s-page";
export interface PageJSXProps extends Partial<PageProps>, Pick<PageProps$1, 'id' | 'children'> {
    /**
     * The main content area of the page, rendered below the header. Accepts any valid elements
     * including cards, forms, lists, or data tables. Content width is determined by the `inlineSize`
     * property. When `inlineSize` is `'base'`, main content appears in the left column with the
     * `aside` slot on the right.
     */
    children?: ComponentChildren;
    /**
     * Slot for supplementary content rendered in a right-side column next to the main content area.
     * Only renders when `inlineSize` is set to `'base'`—on `'small'` or `'large'` layouts, this slot
     * is not displayed. On smaller viewports, `aside` content stacks below the main content. Commonly
     * used for contextual information, related actions, or summary data such as order totals or
     * customer details.
     */
    aside?: ComponentChildren;
    /**
     * Slot for the page's main call-to-action button, rendered in the header on the right side.
     * Accepts a single `s-button` with `variant="primary"`. The button remains visible as users
     * scroll on mobile devices. Commonly used for actions like "Save", "Create", or "Publish".
     */
    primaryAction?: ComponentChildren;
    /**
     * Slot for additional action buttons displayed in the page header, positioned to the left of
     * the primary action. Accepts `s-button` elements with `variant="secondary"` or default variant,
     * and `s-button-group` for grouping related actions. When there are many secondary actions,
     * they can be grouped in a menu. On smaller viewports, actions may collapse into a menu.
     * Commonly used for operations like "Export", "Duplicate", or "Preview".
     */
    secondaryActions?: ComponentChildren;
    /**
     * Slot for navigation breadcrumb links rendered in the header above the heading. Accepts
     * `s-link` elements that allow users to navigate back to parent pages within the app hierarchy.
     * Links display as a breadcrumb trail, with each link typically representing a level in the
     * navigation structure. Useful for multi-step workflows or hierarchical navigation patterns.
     */
    breadcrumbActions?: ComponentChildren;
}

export interface ParagraphProps extends Required<Pick<ParagraphProps$1, 'accessibilityVisibility' | 'fontVariantNumeric' | 'tone' | 'dir' | 'color' | 'lineClamp'>> {
    color: Extract<ParagraphProps$1['color'], 'base' | 'subdued'>;
}

declare abstract class ParagraphBase extends PolarisCustomElement implements Pick<ParagraphProps, 'fontVariantNumeric' | 'lineClamp' | 'tone' | 'color' | 'dir' | 'accessibilityVisibility'> {
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$s]: ParagraphJSXProps & PreactBaseElementPropsWithChildren<Paragraph>;
        }
    }
}

declare const tagName$s = "s-paragraph";
export interface ParagraphJSXProps extends Partial<ParagraphProps>, Pick<ParagraphProps$1, 'id' | 'children'> {
    /**
     * The content of the Paragraph.
     */
    children?: ComponentChildren;
...
export type PasswordFieldProps = PreactFieldProps<Required<PasswordFieldProps$1>['autocomplete']> & Required<Pick<PasswordFieldProps$1, 'defaultValue' | 'details' | 'disabled' | 'error' | 'labelAccessibilityVisibility' | 'minLength' | 'maxLength' | 'label' | 'name' | 'placeholder' | 'readOnly' | 'required' | 'value'>>;
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$r]: PasswordFieldJSXProps & PreactBaseElementProps<PasswordField>;
        }
    }
}

declare const tagName$r = "s-password-field";
export interface PasswordFieldJSXProps extends Partial<PasswordFieldProps>, Pick<PasswordFieldProps$1, 'id'>, FieldReactProps<typeof tagName$r> {
}

export type RequiredBasePopoverProps = Required<PopoverProps$1>;
...
declare class Popover extends PreactPopoverElement<typeof tagName$q> implements PopoverProps {
    constructor();
}
declare global {
    interface HTMLElementTagNameMap {
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$q]: PopoverJSXProps & PreactBaseElementPropsWithChildren<Popover>;
        }
    }
}

declare const tagName$q = "s-popover";
export interface PopoverJSXProps extends Partial<PopoverProps>, Pick<PopoverProps$1, 'id' | 'children'> {
    /**
     * The content displayed inside the popover when it is visible. Accepts any valid elements
     * including text, buttons, forms, or lists. The popover is opened by an activator element
     * (typically a button) that references this popover's `id` via the `commandFor` attribute.
     * Once triggered, the popover automatically positions itself relative to the activator and
     * adjusts placement to remain within the viewport.
     */
    children?: ComponentChildren;
    /**
     * Callback fired immediately when the popover begins to close, before any exit animations
     * start. The popover is still visible at this point. Useful for canceling pending requests
     * or clearing temporary state. For callbacks that should run after the popover is fully
     * hidden, use `onAfterHide` instead.
     */
    onHide?: (event: CallbackEvent<typeof tagName$q>) => void | null;
    /**
     * Callback fired immediately when the popover begins to open, before any entrance animations
     * start. The popover may not be fully visible or positioned at this point. Useful for
     * initializing content or fetching data. For callbacks that should run after the popover
     * is fully visible, use `onAfterShow` instead.
     */
    onShow?: (event: CallbackEvent<typeof tagName$q>) => void | null;
    /**
     * Callback fired after the popover is fully hidden and all exit animations have completed.
     * Focus has been restored to the activator element that triggered the popover. Useful for
     * final cleanup or resetting state.
     */
    onAfterHide?: (event: CallbackEvent<typeof tagName$q>) => void | null;
    /**
     * Callback fired after the popover is fully visible, positioned, and all entrance animations
     * have completed. The popover is interactive and correctly placed relative to its activator.
     * Useful for focusing elements inside the popover or triggering analytics.
     */
    onAfterShow?: (event: CallbackEvent<typeof tagName$q>) => void | null;
    /**
     * Callback fired when the popover's visibility state changes, either from hidden to visible
     * or visible to hidden. Fires at the start of the transition, before animations begin. The
     * event includes `oldState` and `newState` properties indicating the transition direction.
     */
    onToggle?: (event: CallbackToggleEvent<typeof tagName$q>) => void | null;
    /**
     * Callback fired after a visibility toggle completes and all animations have finished. The
     * event includes `oldState` and `newState` properties indicating what transition occurred.
     * Useful for updating external state or triggering analytics after any visibility change.
     */
    onAfterToggle?: (event: CallbackToggleEvent<typeof tagName$q>) => void | null;
}

export type PressButtonBaseProps = Required<Pick<PressButtonProps$1, 'accessibilityLabel' | 'disabled' | 'icon' | 'inlineSize' | 'lang' | 'loading' | 'tone' | 'variant' | 'pressed' | 'defaultPressed'>>;
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$p]: PressButtonJSXProps & PreactBaseElementPropsWithChildren<PressButton>;
        }
    }
}

declare const tagName$p = "s-press-button";
export interface PressButtonJSXProps extends Partial<PressButtonProps>, Pick<PressButtonProps$1, 'children'> {
    /**
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$o]: QueryContainerJSXProps & PreactBaseElementPropsWithChildren<QueryContainer>;
        }
    }
}

declare const tagName$o = "s-query-container";
export interface QueryContainerJSXProps extends Partial<QueryContainerProps$1>, Pick<QueryContainerProps$1, 'id' | 'children'> {
    /**
     * The content of the container.
     */
    children?: ComponentChildren;
...
export interface SearchFieldJSXProps extends Partial<SearchFieldProps>, Pick<TextFieldProps$1, 'id'>, FieldReactProps<typeof tagName$n> {
}
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$m]: SectionJSXProps & PreactBaseElementPropsWithChildren<Section>;
        }
    }
}

declare const tagName$m = "s-section";
export interface SectionJSXProps extends Partial<SectionProps>, Pick<SectionProps$1, 'id' | 'children'> {
    /**
     * The content of the Section.
     */
    children?: ComponentChildren;
...
export interface SelectJSXProps extends Partial<SelectProps>, Pick<SelectProps$1, 'id' | 'children'> {
    /**
     * The options a user can select from.
     *
     * Accepts `Option` and `OptionGroup` components.
...
export type ResponsiveStackProps = MakeResponsivePick<AlignedStackProps, 'gap' | 'rowGap' | 'columnGap' | 'direction'>;
export interface StackProps extends BoxProps, Pick<Required<AlignedStackProps>, 'justifyContent' | 'alignItems' | 'alignContent'> {
    /**
...
export interface StackJSXProps extends Partial<StackProps>, Pick<StackProps$1, 'id' | 'children'> {
    /**
     * The content of the Stack.
     */
    children?: ComponentChildren;
...
export type HeaderFormat = Extract<TableHeaderProps$1['format'], 'base' | 'currency' | 'numeric'>;
export interface TableHeaderProps extends Pick<TableHeaderProps$1, 'listSlot' | 'format'> {
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$h]: Omit<TableJSXProps, 'filters'> & PreactBaseElementPropsWithChildren<Table>;
        }
    }
}

declare const tagName$h = "s-table";
export interface TableJSXProps extends Partial<TableProps>, Pick<TableProps$1, 'id' | 'children' | 'onNextPage' | 'onPreviousPage'> {
    /**
     * The content of the Table.
     */
    children?: ComponentChildren;
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$g]: TableBodyJSXProps & PreactBaseElementPropsWithChildren<TableBody>;
        }
    }
}

declare const tagName$g = "s-table-body";
export interface TableBodyJSXProps extends Partial<TableBodyProps>, Pick<TableBodyProps$1, 'id' | 'children'> {
    /**
     * The body of the table. May not have any semantic meaning in the Table's `list` variant.
     */
    children?: ComponentChildren;
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$f]: TableCellJSXProps & PreactBaseElementPropsWithChildren<TableCell>;
        }
    }
}

declare const tagName$f = "s-table-cell";
export interface TableCellJSXProps extends Partial<TableCellProps>, Pick<TableCellProps$1, 'id' | 'children'> {
    /**
     * The content of the table cell.
     */
    children?: ComponentChildren;
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$e]: TableHeaderJSXProps & PreactBaseElementPropsWithChildren<TableHeader>;
        }
    }
}

declare const tagName$e = "s-table-header";
export interface TableHeaderJSXProps extends Partial<TableHeaderProps>, Pick<TableHeaderProps$1, 'id' | 'children'> {
    /**
     * The heading of the column in the `table` variant, and the label of its data in `list` variant.
     */
    children?: ComponentChildren;
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$d]: TableHeaderRowJSXProps & PreactBaseElementPropsWithChildren<TableHeaderRow>;
        }
    }
}

declare const tagName$d = "s-table-header-row";
export interface TableHeaderRowJSXProps extends Partial<TableHeaderRowProps>, Pick<TableHeaderRowProps$1, 'id' | 'children'> {
    /**
     * Contents of the table heading row; children should be `TableHeading` components.
     */
    children?: ComponentChildren;
...
export interface TableRowProps extends Pick<TableRowProps$1, 'children' | 'clickDelegate'> {
}
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$c]: TableRowJSXProps & PreactBaseElementPropsWithChildren<TableRow>;
        }
    }
}

declare const tagName$c = "s-table-row";
export interface TableRowJSXProps extends Partial<TableRowProps>, Pick<TableRowProps$1, 'id' | 'children'> {
    /**
     * The content of a TableRow, which should be `TableCell` components.
     */
    children?: ComponentChildren;
...
export interface TextProps extends Required<Pick<TextProps$1, 'accessibilityVisibility' | 'dir' | 'color' | 'type' | 'tone' | 'fontVariantNumeric' | 'interestFor'>> {
...
export interface TextJSXProps extends Partial<TextProps>, Pick<TextProps$1, 'id' | 'children'> {
    /**
     * The content of the Text.
     */
    children?: ComponentChildren;
...
export interface TextAreaJSXProps extends Partial<TextAreaProps>, Pick<TextAreaProps$1, 'id'>, FieldReactProps<typeof tagName$a> {
}
...
declare abstract class TextFieldBase extends PreactFieldElement<TextFieldProps['autocomplete']> implements Pick<TextFieldProps, 'icon' | 'maxLength' | 'minLength' | 'prefix' | 'suffix'> {
    /**
     * The icon displayed inside the field, typically at the start. Provides visual context about
     * the expected input type. Available icons include `'search'`, `'email'`, and others from the
     * Polaris icon set.
     */
    icon: TextFieldProps['icon'];
    /**
     * The maximum number of characters allowed in the field. When set, a character counter may
     * appear and input is limited to this length. Set to `Infinity` (the default) for no limit.
     *
     * @default Infinity
     */
    maxLength: TextFieldProps['maxLength'];
    /**
     * The minimum number of characters required for valid input. The field will fail validation
     * if the value contains fewer characters than this number.
     *
     * @default 0
     */
    minLength: TextFieldProps['minLength'];
    /**
     * Content displayed before the input value, such as currency symbols (`'$'`) or other static
     * text. Renders inside the field but is not part of the editable or submitted value.
     */
    prefix: TextFieldProps['prefix'];
    /**
     * Content displayed after the input value, such as units (`'kg'`, `'%'`) or domain endings
     * (`'.myshopify.com'`). Renders inside the field but is not part of the editable or submitted
     * value.
     */
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$9]: Omit<TextFieldJSXProps, 'accessory'> & PreactBaseElementPropsWithChildren<TextField>;
        }
    }
}

declare const tagName$9 = "s-text-field";
export interface TextFieldJSXProps extends Partial<Omit<TextFieldProps, 'accessory'>>, Pick<TextFieldProps$1, 'id'>, FieldReactProps<typeof tagName$9> {
    /**
...
export interface ThumbnailProps extends Required<Pick<ThumbnailProps$1, 'src' | 'alt' | 'size'>> {
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$7]: TooltipJSXProps & PreactBaseElementPropsWithChildren<Tooltip>;
        }
    }
}

declare const tagName$7 = "s-tooltip";
export interface TooltipJSXProps extends Partial<TooltipProps>, Pick<TooltipProps$1, 'id' | 'children'> {
    /**
     * The content of the Tooltip.
     *
...
export type URLFieldProps = PreactFieldProps<Required<URLFieldProps$1>['autocomplete']> & Required<Pick<URLFieldProps$1, 'maxLength' | 'minLength'>>;
...
export interface URLFieldJSXProps extends Partial<Omit<URLFieldProps, 'accessory'>>, Pick<URLFieldProps$1, 'id'>, FieldReactProps<typeof tagName$6> {
}
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$5]: UnorderedListJSXProps & PreactBaseElementPropsWithChildren<UnorderedList>;
        }
    }
}
...
declare class AdminAction extends PolarisCustomElement implements AdminActionProps {
    heading: string;
    loading: boolean;
    constructor();
}
...
declare module 'preact' {
    namespace createElement.JSX {
        interface IntrinsicElements {
            [tagName$4]: Omit<AdminActionJSXProps, 'primaryAction' | 'secondaryActions'> & {
                children?: preact.ComponentChildren;
            };
        }
    }
...
declare class AdminBlock extends PolarisCustomElement implements AdminBlockProps {
    heading: string;
    collapsedSummary: string;
    constructor();
}
...
export interface AdminPrintActionProps extends Pick<AdminPrintActionProps$1, 'src'> {
}

declare const tagName$2 = "s-admin-print-action";
export interface AdminPrintActionJSXProps extends Partial<AdminPrintActionProps>, Pick<AdminPrintActionProps$1, 'id'> {
}

declare class AdminPrintAction extends PolarisCustomElement implements AdminPrintActionProps {
    src: string;
    constructor();
}
declare global {
...
export interface FormJSXProps extends Partial<FormProps> {
     ...
     * A callback that is run when the form is submitted.
     */
    onSubmit?: ((event: CallbackExtendableEvent<typeof tagName$1>) => void) | null;
    /**
     * A callback that is run when the form is reset.
     */
...
export interface FunctionSettingsProps extends Pick<FunctionSettingsProps$1, 'id'> {
}

declare const tagName = "s-function-settings";
export interface FunctionSettingsJSXProps extends Partial<FunctionSettingsProps & Pick<FunctionSettingsProps$1, 'onError'>> {
    /**
     * An optional callback function that will be run by the admin when the user
     * commits their changes in the admin-rendered part of the function settings
     * experience. If `event.waitUntil` is called with a promise, the admin will wait for the
...
export type FunctionSettingsErrorEvent = Parameters<NonNullable<FunctionSettingsProps$1['onError']>>[0];
declare class FunctionSettings extends PreactCustomElement implements FunctionSettingsProps {
    constructor();
}
declare global {
    interface HTMLElementTagNameMap {
...
export { AdminAction, AdminBlock, AdminPrintAction, Avatar, Badge, Banner, Box, Button, ButtonGroup, Checkbox, Chip, Choice, ChoiceList, Clickable, ClickableChip, ColorField, ColorPicker, DateField, DatePicker, Divider, DropZone, EmailField, Form, FunctionSettings, Grid, GridItem, Heading, Icon, Image, Link, ListItem, Menu, Modal, MoneyField, NumberField, Option, OptionGroup, OrderedList, Page, Paragraph, PasswordField, Popover, PressButton, QueryContainer, SearchField, Section, Select, Spinner, Stack, Switch, Table, TableBody, TableCell, TableHeader, TableHeaderRow, TableRow, Text, TextArea, TextField, Thumbnail, Tooltip, URLField, UnorderedList };
export type { AdminActionJSXProps, AdminBlockJSXProps, AdminPrintActionJSXProps, ReactProps$_AvatarJSXProps, BadgeJSXProps, BannerJSXProps, BoxJSXProps, ButtonGroupJSXProps, ButtonJSXProps, CheckboxJSXProps, ChipJSXProps, ChoiceJSXProps, ChoiceListJSXProps, ClickableChipJSXProps, ClickableJSXProps, ColorFieldJSXProps, ColorPickerJSXProps, DateFieldJSXProps, DatePickerJSXProps, DividerJSXProps, DropZoneJSXProps, EmailFieldJSXProps, FormJSXProps, FunctionSettingsJSXProps, GridItemJSXProps, GridJSXProps, HeadingJSXProps, IconJSXProps, ImageJSXProps, LinkJSXProps, ListItemJSXProps, MenuJSXProps, ModalJSXProps, MoneyFieldJSXProps, NumberFieldJSXProps, OptionGroupJSXProps, OptionJSXProps, OrderedListJSXProps, PageJSXProps, ParagraphJSXProps, PasswordFieldJSXProps, PopoverJSXProps, PressButtonJSXProps, QueryContainerJSXProps, SearchFieldJSXProps, SectionJSXProps, SelectJSXProps, SpinnerJSXProps, StackJSXProps, SwitchJSXProps, TableBodyJSXProps, TableCellJSXProps, TableHeaderJSXProps, TableHeaderRowJSXProps, TableJSXProps, TableRowJSXProps, TextAreaJSXProps, TextFieldJSXProps, TextJSXProps, ThumbnailJSXProps, TooltipJSXProps, URLFieldJSXProps, UnorderedListJSXProps };
...
export interface DropZoneEvents {
     ...
     * file selection—useful for processing uploads, updating UI state, or triggering validation.
     */
    change: CallbackEventListener<typeof tagName$J>;
    /**
     * Callback fired during file input. Similar to `onchange` but might fire more frequently during
     * the selection process. Provided for compatibility with form libraries that expect standard
     * input events.
     */
    input: CallbackEventListener<typeof tagName$J>;
    /**
     * Callback fired when a user attempts to drop or select files that don't match the `accept`
     * criteria. The event provides information about which files were rejected and why. Useful
     * for displaying specific error messages like "Only PNG and JPEG images are accepted".
     */
    droprejected: CallbackEventListener<typeof tagName$J>;
}
...
declare module 'react' {
    namespace JSX {
        interface IntrinsicElements {
            [tagName$Y]: Omit<BannerJSXProps, 'secondaryActions'> & ReactBaseElementPropsWithChildren<Banner>;
        }
    }
}
declare global {
...
declare module 'react' {
    namespace JSX {
        interface IntrinsicElements {
            [tagName$V]: Omit<ButtonGroupJSXProps, 'primaryAction' | 'secondaryActions'> & ReactBaseElementPropsWithChildren<ButtonGroup>;
        }
    }
}
declare global {
...
declare module 'react' {
    namespace JSX {
        interface IntrinsicElements {
            [tagName$T]: Omit<ChipJSXProps, 'graphic'> & ReactBaseElementPropsWithChildren<Chip>;
        }
    }
}
declare global {
...
declare module 'react' {
    namespace JSX {
        interface IntrinsicElements {
            [tagName$S]: Omit<ChoiceJSXProps, 'details' | 'secondaryContent'> & ReactBaseElementPropsWithChildren<Choice>;
        }
    }
}
declare global {
...
declare module 'react' {
    namespace JSX {
        interface IntrinsicElements {
            [tagName$R]: ChoiceListJSXProps & ReactBaseElementPropsWithChildren<ChoiceList>;
        }
    }
}
declare global {
...
declare module 'react' {
    namespace JSX {
        interface IntrinsicElements {
            [tagName$Q]: ClickableJSXProps & ReactBaseElementPropsWithChildren<Clickable>;
        }
    }
}
declare global {
...
declare module 'react' {
    namespace JSX {
        interface IntrinsicElements {
            [tagName$P]: Omit<ClickableChipJSXProps, 'graphic'> & ReactBaseElementPropsWithChildren<ClickableChip>;
        }
    }
}
declare global {
...
declare module 'react' {
    namespace JSX {
        interface IntrinsicElements {
            [tagName$J]: DropZoneJSXProps & ReactBaseElementPropsWithChildren<DropZone>;
        }
    }
}
declare global {
...
declare module 'react' {
    namespace JSX {
        interface IntrinsicElements {
            [tagName$G]: GridItemJSXProps & ReactBaseElementPropsWithChildren<GridItem>;
        }
    }
}
declare global {
...
declare module 'react' {
    namespace JSX {
        interface IntrinsicElements {
            [tagName$B]: ListItemJSXProps & ReactBaseElementPropsWithChildren<ListItem>;
        }
    }
}
declare global {
...
declare module 'react' {
    namespace JSX {
        interface IntrinsicElements {
            [tagName$z]: Omit<ModalJSXProps, 'primaryAction' | 'secondaryActions'> & ReactBaseElementPropsWithChildren<Modal>;
        }
    }
}
declare global {
...
declare module 'react' {
    namespace JSX {
        interface IntrinsicElements {
            [tagName$v]: OptionGroupJSXProps & ReactBaseElementPropsWithChildren<OptionGroup>;
        }
    }
}
declare global {
...
declare module 'react' {
    namespace JSX {
        interface IntrinsicElements {
            [tagName$u]: OrderedListJSXProps & ReactBaseElementPropsWithChildren<OrderedList>;
        }
    }
}
declare global {
...
declare module 'react' {
    namespace JSX {
        interface IntrinsicElements {
            [tagName$t]: Omit<PageJSXProps, 'aside' | 'primaryAction' | 'secondaryActions' | 'breadcrumbActions'> & ReactBaseElementPropsWithChildren<Page>;
        }
    }
}
declare global {
...
declare module 'react' {
    namespace JSX {
        interface IntrinsicElements {
            [tagName$s]: ParagraphJSXProps & ReactBaseElementPropsWithChildren<Paragraph>;
        }
    }
}
declare global {
...
declare module 'react' {
    namespace JSX {
        interface IntrinsicElements {
            [tagName$p]: PressButtonJSXProps & ReactBaseElementPropsWithChildren<PressButton>;
        }
    }
}
declare global {
...
declare module 'react' {
    namespace JSX {
        interface IntrinsicElements {
            [tagName$o]: QueryContainerJSXProps & ReactBaseElementPropsWithChildren<QueryContainer>;
        }
    }
}
declare global {
...
declare module 'react' {
    namespace JSX {
        interface IntrinsicElements {
            [tagName$h]: Omit<TableJSXProps, 'filters'> & ReactBaseElementPropsWithChildren<Table>;
        }
    }
}
declare global {
...
declare module 'react' {
    namespace JSX {
        interface IntrinsicElements {
            [tagName$g]: TableBodyJSXProps & ReactBaseElementPropsWithChildren<TableBody>;
        }
    }
}
declare global {
...
declare module 'react' {
    namespace JSX {
        interface IntrinsicElements {
            [tagName$f]: TableCellJSXProps & ReactBaseElementPropsWithChildren<TableCell>;
        }
    }
}
declare global {
...
declare module 'react' {
    namespace JSX {
        interface IntrinsicElements {
            [tagName$e]: TableHeaderJSXProps & ReactBaseElementPropsWithChildren<TableHeader>;
        }
    }
}
declare global {
...
declare module 'react' {
    namespace JSX {
        interface IntrinsicElements {
            [tagName$d]: TableHeaderRowJSXProps & ReactBaseElementPropsWithChildren<TableHeaderRow>;
        }
    }
}
declare global {
...
declare module 'react' {
    namespace JSX {
        interface IntrinsicElements {
            [tagName$c]: TableRowJSXProps & ReactBaseElementPropsWithChildren<TableRow>;
        }
    }
}
declare global {
...
declare module 'react' {
    namespace JSX {
        interface IntrinsicElements {
            [tagName$9]: Omit<TextFieldJSXProps, 'accessory'> & ReactBaseElementPropsWithChildren<TextField>;
        }
    }
}
declare global {
...
declare module 'react' {
    namespace JSX {
        interface IntrinsicElements {
            [tagName$5]: UnorderedListJSXProps & ReactBaseElementPropsWithChildren<UnorderedList>;
        }
    }
}
declare global {
