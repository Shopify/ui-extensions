...
`,
  ...
  examples: {
    ...
    examples: [
      ...
      {
        description:
          'Open the picker with specific products already preselected. Uses `selectionIds` array with product GIDs like `gid://shopify/Product/123` to mark items as selected—useful for edit workflows.',
        codeblock: {
      ...
      {
        description:
          'Customize the picker\'s confirm button with an action verb. Sets `action: "add"` to change button text from "Select" to "Add", helping merchants understand whether they\'re adding to a list or replacing.',
        codeblock: {
      ...
      {
        description:
          'Allow unlimited selection or limit to a maximum count. Set `multiple: true` for unlimited or `multiple: 5` to cap at 5 items—the picker automatically prevents selection beyond the limit.',
        codeblock: {
      ...
      {
        description:
          'Provide an initial search query to filter picker results. Uses `query` property to pre-fill the search field with a term like "shirt"—helpful for guiding merchants to specific items in large catalogs.',
        codeblock: {
      ...
      {
        description:
          'Access selected resource data including IDs, titles, and images. Resource picker returns an array of resources with full metadata, or `undefined` if the merchant cancels.',
        codeblock: {
