...
> If you need to pick Shopify products, variants, or collections, use the [Resource Picker API](/docs/api/admin-extensions/{API_VERSION}/target-apis/utility-apis/resource-picker-api) instead.`,
  ...
  defaultExample: {
    description:
      'Build a custom picker for email templates with columns and badges. This example defines table headers (Templates, Created by, Times used), populates items with data fields, and adds status badges (Draft/Published) to distinguish template states.',
    image: 'picker.png',
    codeblock: {
