  ...
  definitions: [
    {
      ...
      description:
        'The `BlockExtensionApi` object provides methods for block extensions that render inline content on admin pages. Access these properties through the API to interact with the current context, navigate to other extensions, and display the resource picker.',
  ...
  subSections: [
    {
      ...
      sectionContent:
        ...
        '- **Use navigation and resource picker effectively:** Use the `api.navigation` property to navigate between extensions and the [Resource Picker](/docs/api/admin-extensions/{API_VERSION}/target-apis/utility-apis/resource-picker-api) API for resource selection to create seamless workflows within your block.',
    },
    {
      type: 'Generic',
