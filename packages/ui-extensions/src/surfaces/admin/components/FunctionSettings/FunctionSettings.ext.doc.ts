  ...
  definitions: [
    {
      title: 'Events',
      description:
        'The function settings component provides event callbacks for handling user interactions. Learn more about [handling events](/docs/api/app-ui/using-web-components#handling-events).',
      type: 'FormEvents',
    },
  ],
  defaultExample: {
    description:
      'Configure a [Shopify Function](/docs/api/functions/) with a settings form that integrates with the native save bar. This example shows a [number field](/docs/api/admin-extensions/{API_VERSION}/web-components/forms/number-field) for a discount percentage input inside a function settings component.',
    codeblock: {
    ...
    exampleGroups: [
      {
        ...
        examples: [
          {
            description:
              'Organize settings into repeating groups for tiered or multi-level configurations. This example uses [sections](/docs/api/admin-extensions/{API_VERSION}/web-components/layout-and-structure/section) to define two volume discount tiers, each with a quantity threshold and percentage.',
            codeblock: {
          ...
          {
            description:
              'Display validation errors when settings values are out of range or missing. This example shows required [number fields](/docs/api/admin-extensions/{API_VERSION}/web-components/forms/number-field) with inline error messages.',
            codeblock: {
