...
export interface StackProps
  ...
  /**
   * The spacing between children in the `Stack`. You can provide a single [SpacingKeyword](https://shopify.dev/docs/api/app-home/using-polaris-components#scale) value to apply the same spacing to both axes (for example, `'large-100'`), or a pair of values (for example, `'large-100 large-500'`) to set different spacing for the block and inline axes. This property also accepts [responsive values](https://shopify.dev/docs/api/polaris/using-polaris-web-components#responsive-values) using container query syntax.
   *
   * @default 'none'
   */
  ...
  /**
   * The direction in which the `Stack`'s children are laid out. Use `'inline'` to arrange children horizontally (with wrapping enabled), or `'block'` to arrange them vertically (without wrapping). This property also accepts [responsive values](https://shopify.dev/docs/api/polaris/using-polaris-web-components#responsive-values) using container query syntax.
   *
   * @default 'block'
   *
