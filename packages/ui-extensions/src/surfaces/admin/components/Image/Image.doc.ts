...
const data: ReferenceEntityTemplateSchema = {
  ...
  examples: {
    ...
    examples: [
      {
        description:
          'Handle image loading and error states using `onLoad` and `onError` callbacks. This example shows a [Text](/docs/api/admin-extensions/{API_VERSION}/components/typography-and-content/text) loading message while the image loads and displays an error message if the image fails to load, preventing broken image placeholders.',
        codeblock: {
