 ...
 * HTML heading level is determined by `HeadingGroup` nesting.
 */
type Level = 1 | 2 | 3 | 4 | 5 | 6;

/**
 * Props for the `Heading` component. The heading's semantic level is
 * determined automatically by the nesting depth of `HeadingGroup`
 * ancestors; use the `size` prop to control its visual appearance
 * independently.
 */
export interface HeadingProps {
   ...
   * automatically by the nesting depth of parent `HeadingGroup` components.
   */
  size?: Level;
}
 ...
 * by the nesting depth of `HeadingGroup` ancestors.
 */
