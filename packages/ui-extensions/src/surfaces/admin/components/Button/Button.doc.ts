...
- **Establish clear hierarchy:** Use \`primary\` variant for the main action, \`secondary\` for supporting actions, and \`tertiary\` for supplementary or low-emphasis actions. Avoid placing multiple primary buttons in the same view.
- **Position consistently:** Place buttons in predictable locations. In [\`AdminAction\`](/docs/api/admin-extensions/{API_VERSION}/ui-components/settings-and-templates/adminaction) components, use the \`primaryAction\` and \`secondaryAction\` properties. In forms, place submit buttons at the bottom of the form.
- **Provide accessibility labels for icon-only usage:** When using a \`Button\` without visible text content, always set \`accessibilityLabel\` so screen reader users understand the button's purpose.`,
    },
    {
      type: 'Generic',
      title: 'Limitations',
      anchorLink: 'limitations',
      sectionContent: `- The \`Button\` component doesn't include a built-in loading state. To indicate that an async operation is in progress, consider disabling the button and displaying a loading indicator nearby.
- When using \`href\` for navigation, external URLs (domains outside the Shopify admin) may be blocked or trigger a confirmation dialog depending on the extension context and browser security settings.
