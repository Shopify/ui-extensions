  ...
  subSections: [
    {
      ...
      sectionContent: `- **Design for square cropping:** Thumbnails automatically crop images to a 1:1 aspect ratio from the center. If your images aren't square, important content near the edges may be cut off.
...
- **Choose appropriate sizes for your context:** Smaller thumbnails work better in dense layouts like tables, while larger sizes suit product-focused interfaces. Consider the merchant's task and the information density when choosing a size.`,
    },
    {
      ...
      sectionContent: `- Thumbnails always render as 1:1 squares and will crop non-square images to fit. The component uses center cropping, which may cut off important image details.
- Images must be served from publicly accessible URLs with appropriate CORS headers for cross-origin requests. Local files and data URLs aren't supported.
...
- Thumbnails don't include built-in lazy loading. In long lists with many thumbnails, all images load immediately, which may impact performance.`,
    },
  ],
  definitions: [
    {
