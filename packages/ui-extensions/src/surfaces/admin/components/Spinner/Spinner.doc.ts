  ...
  subSections: [
    {
      ...
      sectionContent: `- **Show for asynchronous operations:** Display spinners when loading takes longer than 500ms for operations like saving forms, fetching data, or processing actions. For instant actions, spinners may flash distractingly.
...
- **Pair with action feedback:** After a spinner completes, show clear success or error messages so users know the operation finished and its outcome.`,
    },
    {
      title: 'Limitations',
      type: 'Generic' as const,
...
- The component itself doesn't provide a way to cancel the loading operation. If users need to cancel, you must implement separate UI controls like a **Cancel** [\`Button\`](/docs/api/{API_NAME}/{API_VERSION}/polaris-web-components/actions/button) alongside the spinner.
- The \`accessibilityLabel\` is set once and doesn't update. For multi-stage operations, you must unmount and remount the spinner with a new label, or use ARIA live regions for dynamic status updates.`,
    },
  ],
  definitions: [
    {
