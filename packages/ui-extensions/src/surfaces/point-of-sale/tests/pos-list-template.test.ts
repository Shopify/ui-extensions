...
describe('posListTemplate', () => {
  ...
  describe('conditionals', () => {
    ...
    it('rejects a condition nested through component children', () => {
      expect(() =>
        compileItem(
          '{{#if outer}}<s-box><s-stack>{{#if inner}}<s-text>Visible</s-text>{{/if}}</s-stack></s-box>{{/if}}',
        ),
      ).toThrow(/cannot be nested/);
    });

    it('allows a separate condition after an enclosing condition closes', () => {
      const result = compileItem(
        '{{#if first}}<s-box><s-text>First</s-text></s-box>{{/if}}<s-stack>{{#if second}}<s-text>Second</s-text>{{/if}}</s-stack>',
      );

      expect(result.templates[0]?.children).toMatchObject([
        {
          kind: 'if',
          path: 'first',
          children: [{kind: 'element', tag: 's-box'}],
        },
        {
          kind: 'element',
          tag: 's-stack',
          children: [{kind: 'if', path: 'second'}],
        },
      ]);
    });
