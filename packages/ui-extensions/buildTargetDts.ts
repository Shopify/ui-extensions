...
function extractSurfaceTypesAndComponents(
  ...
  // 5. Scan for component prop interfaces
  scanComponentsForPropInterfaces(srcPath, project, componentProps);

  return {
    apis: Array.from(apis),
    components: Array.from(components),
    componentProps: Array.from(componentProps),
  };
}
...
function scanComponentsForPropInterfaces(
  srcPath: string,
  project: Project,
  componentProps: Set<string>,
) {
  // Scan the components directory for all .d.ts files
  const componentsDir = resolve(srcPath, 'components');
  ...
  const scanDir = (dir: string) => {
    ...
    items.forEach((item) => {
      if (item.isDirectory()) {
      ...
      } else if (item.name.endsWith('.d.ts') && !item.name.includes('shared')) {
        ...
        try {
          ...
          // Extract all exported interfaces ending with 'Props' or 'Element'
          sourceFile
            .getInterfaces()
            .filter((iface) => {
              const name = iface.getName();
              return (
                iface.isExported() &&
                (name.endsWith('Props') || name.endsWith('Element'))
              );
            })
            .forEach((iface) => {
              componentProps.add(iface.getName());
            });

          // Also extract type aliases ending with 'Props' or 'Element'
          sourceFile
            .getTypeAliases()
            .filter((alias) => {
              const name = alias.getName();
            ...
            .forEach((alias) => {
              componentProps.add(alias.getName());
            });
        } catch (error) {
          // Skip files that can't be parsed
        }
...
function createSurfaceGlobalDeclarations({
  ...
  const {apis, components, componentProps} = extractSurfaceTypesAndComponents(
    surface,
    project,
    directory,
  );
  ...
  // Create component props type union for this surface
  const surfaceComponentPropsType =
    componentProps.length > 0 ? componentProps.join(' | ') : 'never';
...
declare global {
  ...
  namespace ShopifyComponents {
    namespace ${capitalizeFirst(surface)} {
      ...
      type AllComponentProps = ${surfaceComponentPropsType};
    }
  }
}
...
`;
  ...
  console.log(
    `Created global declarations for ${surface} surface with APIs: ${apis.join(
      ', ',
    )}, Components: ${components.join(', ')}, and Component Props: ${componentProps.length} types`,
  );
}
