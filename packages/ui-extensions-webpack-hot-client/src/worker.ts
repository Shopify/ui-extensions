...
function run() {
  ...
  if (host === '*') {
    throw new Error(
      `@shopify/ui-extensions-webpack-hot-client does not support setting the host to "*", because the window’s location is not guaranteed to be available in a worker.`,
    );
  }
