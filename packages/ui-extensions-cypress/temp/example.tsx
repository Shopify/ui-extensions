...
function WorkerApp({user}: {user: User}) {
  ...
  return (
    <Card>
      Welcome, user {user.id}! You’ve clicked {count}{' '}
      {count === 1 ? 'time' : 'times'} (from a worker!){' '}
      <Button onPress={() => setCount((count) => count + 1)}>Plus one</Button>{' '}
      <Button onPress={() => user.getDetails().then(log)}>
        Fetch user details
      </Button>{' '}
      <Button
        onPress={() =>
          (self as any).authenticatedFetch('/products.json').then(log)
        }
      >
        Authenticated fetch
      </Button>
